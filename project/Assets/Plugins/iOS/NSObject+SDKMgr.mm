...
extern "C" {
    ...
    char *OnGameMessageRet(char *param)
    {
        printf(param);
        NSString *ret =[[SDKApi Instance]OnGameMesssageRet:CreateNSString(param)];
        const char * cret = [ret cStringUsingEncoding:NSUTF8StringEncoding];
        printf(cret);
        return strdup(cret);
    }
    
    void OnGameLaunch(void)
    {
