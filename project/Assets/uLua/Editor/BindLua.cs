...
public static class LuaBinding
{
    ...
    static void GenLuaBinder()
    {
        ...
        gen.DoStatement("//");
        gen.DoStatement("// 该文件自动生成，没事别乱改。");
        gen.DoStatement("//");
        gen.DoStatement("using System;");
        gen.DoStatement("using System.Collections.Generic;");
        gen.DoStatement("");
        gen.DoStatement("public static class LuaBinder");
        gen.BeginBlock("{");
        gen.DoStatement("private static List<string> wrapList = new List<string>();");
        gen.DoStatement("");
        gen.DoStatement("public static bool IsBinded(string type) { return wrapList.Contains(type); }");
        gen.DoStatement("");
        gen.DoStatement("public static void Clear() { wrapList.Clear(); }");
        gen.DoStatement("");
        gen.DoStatement("public static System.Type Bind(IntPtr L, string typeName = null)");
        gen.BeginBlock("{");
        gen.DoStatement("System.Type type = null;");
        gen.BeginBlock("switch (typeName) {0}", '{');
        ...
        for (int i = 0; i < files.Length; i++) {
            ...
            var className = wrapName.Replace('_', '.').Substring(0, wrapName.Length - 4);
            gen.DoStatement("case \"{0}\": type = {1}.Register(L); break;", className, wrapName);

            wrapfiles.Add("import '" + className + "'");
        }
        ...
        gen.DoStatement("default : break;");
        gen.EndBlock("}");
        gen.DoStatement("wrapList.Add(typeName);");
        gen.DoStatement("return type;");
        gen.EndBlock("}");
        gen.EndBlock("}");
