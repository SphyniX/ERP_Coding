...
public static class WrapToLua
{
        ...
        if (fields != null) {
            for (int i = 0; i < fields.Length; i++) {
                ...
                if (fields[i].getter != null) {
                    self.PushCSharpFunction(fields[i].getter);
                    self.RawSetI(-2, 1);
                }

                if (fields[i].setter != null) {
                    self.PushCSharpFunction(fields[i].setter);
                    self.RawSetI(-2, 2);
                }
    ...
    public static void WrapCSharpObject(this ILuaState self, System.Type type, LuaMethod[] methods, LuaField[] fields)
    {
        ...
        // 元方法放入元表
        LuaDLL.tolua_setindex(self);
        LuaDLL.tolua_setnewindex(self);
        self.SetDict("__tostring", new LuaCSFunction(MetaMethods.__tostring));
        self.SetDict("__gc", new LuaCSFunction(MetaMethods.__gc));
        self.SetDict("__eq", new LuaCSFunction(MetaMethods.__eq));
    ...
    /// 调用后需要设置元表为类表的元表
    /// </summary>
    public static void WrapCSharpObject(this ILuaState self, System.Type type, LuaMethod[] methods)
    {
        // 保存类型
        var metaName = self.RegistType(type);
        ...
        self.RegistMembers(methods, null);
    }
